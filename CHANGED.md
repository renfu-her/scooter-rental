...
## 2026-01-20 10:44:47 (Asia/Taipei) - 調整 Excel 報表中店名顯示位置
...
- **PartnerMonthlyReportExport.php** (`app/Exports/PartnerMonthlyReportExport.php`)
  - 調整店名顯示位置，從標題下方換行改為標題右側獨立顯示
  - 標題「蘭光智能出租月報表」佔據大部分列（A 列到倒數第二列）
  - 店名「店家：{店名}」顯示在最後一列（右側），獨立儲存格
  - 店名使用左對齊，字體大小 12，粗體
  - 標題和店名在同一行，不影響後續數據列的結構

### 功能說明

- 現在 Excel 報表中，店名會顯示在標題行的右側，格式為「店家：{店名}」
- 店名顯示在獨立的儲存格中，不會與標題合併
- 後續的數據列結構保持不變，不受影響
